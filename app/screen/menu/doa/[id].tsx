  ...
  return (
    <SafeAreaView className="flex-1 bg-[#fbf5ea] dark:bg-[#0b1220]" edges={["top"]}>
      <View className="px-4 py-3 border-b border-[#e5e5e5] dark:border-[#1f2937] flex-row items-center justify-between">
        ...
        <Text className="text-xl font-bold text-[#363636] dark:text-[#f8fafc]">Detail Doa</Text>
        <TouchableOpacity
        ...
        >
          ...
          {isUsingCache ? (
            <View className="rounded-xl bg-[#dbe8e8] border border-[#bfd1d1] px-3 py-2 mb-3">
              <Text className="text-[#336363] dark:text-[#9fb7b7] text-xs">
                Detail ini ditampilkan dari cache offline.
              </Text>
            </View>
          ) : null}

          <View className="rounded-2xl bg-white dark:bg-[#111827] border border-[#e5e5e5] dark:border-[#1f2937] p-5">
            <Text className="text-[#64748b] text-xs font-semibold">Doa #{item.id}</Text>
            <Text className="text-[#1f2937] dark:text-[#e5e7eb] font-bold text-xl mt-2">{item.title}</Text>

            <View className="h-px bg-[#ececec] my-4" />

            <Text className="text-[#4b5563] dark:text-[#cbd5e1] text-xs font-semibold uppercase">Arab</Text>
            <Text
              className="text-right text-[30px] leading-[44px] text-[#111827] mt-2 font-arabic"
              style={{ writingDirection: "rtl" }}
            >
              {item.arabic}
            </Text>

            <Text className="text-[#4b5563] dark:text-[#cbd5e1] text-xs font-semibold uppercase mt-5">
              Latin
            </Text>
            <Text className="text-[#374151] dark:text-[#e5e7eb] text-base mt-2 leading-7">{item.latin}</Text>

            <Text className="text-[#4b5563] dark:text-[#cbd5e1] text-xs font-semibold uppercase mt-5">
              Terjemahan
            </Text>
            <Text className="text-[#374151] dark:text-[#e5e7eb] text-base mt-2 leading-7">
              {item.translation}
            </Text>
            ...
            {item.source ? (
              <>
                ...
                <Text className="text-[#4b5563] dark:text-[#cbd5e1] text-sm mt-2">{item.source}</Text>
              </>
            ) : null}
          </View>
        </ScrollView>
