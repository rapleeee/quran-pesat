  ...
  return (
    <SafeAreaView className="flex-1 bg-[#fbf5ea] dark:bg-[#0b1220]" edges={["top", "bottom"]}>
      <View className="px-4 py-3 border-b border-[#e5e5e5] dark:border-[#1f2937] flex-row items-center justify-between">
        ...
        >
          <ArrowLeft size={22} color="#728d8d" />
        </TouchableOpacity>
        <Text className="text-lg font-bold text-[#363636] dark:text-[#f8fafc]">Panduan Voice Arab</Text>
      ...
      >
        <View className="rounded-2xl border border-[#e5e7eb] dark:border-[#1f2937] bg-white dark:bg-[#111827] p-4">
          <Text className="text-[#111827] text-base font-semibold">
            Kenapa suara belum Arab?
          </Text>
          <Text className="mt-2 text-[#4b5563] dark:text-[#cbd5e1] text-sm leading-6">
            Aplikasi sudah meminta bacaan bahasa Arab, tapi sistem perangkat tetap butuh
            paket voice Arabic untuk hasil yang benar.
          </Text>
        </View>

        <View className="mt-3 rounded-2xl border border-[#e5e7eb] dark:border-[#1f2937] bg-white dark:bg-[#111827] p-4">
          ...
          {steps.map((step, index) => (
            <View key={step} className="mt-3 flex-row items-start">
              <View className="w-6 h-6 rounded-full bg-[#ecf1f1] items-center justify-center mr-2 mt-0.5">
                <Text className="text-[#336363] dark:text-[#9fb7b7] text-xs font-semibold">{index + 1}</Text>
              </View>
              <Text className="flex-1 text-[#374151] dark:text-[#e5e7eb] text-sm leading-6">{step}</Text>
            </View>
          ))}
        </View>
